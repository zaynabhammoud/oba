...
public class Alignment {
     ...
     * concept. Otherwise, it will return null. For the direct comparisons, it
     * uses the ISUB similarity measure which has a clear superiority in
     * ontology alignment name searching over other lexical measures and
     * specially designed for ontology alignment.
     *
    ...
    protected String findLexicalSimilar(String SimString,
        ...
        for (String OntRes : OntSearchList) {
            double Sim = s.ISUBSimilarity(SimString, OntRes);
     ...
     * measure. If the similarity could not be determined, this method has not
     * enough information to reject the similarity(showed in method by -1). If
     * the proposed similar concepts have a semantic similarity lower than a
     * threshold, the method will reject the similarity. Otherwise, the method
     * accepts their similarity.
     *
     ...
     * synonyms and hypernyms to ontology concepts, we simply use the
     * Levenshtein similarity measure since we know the ISUB failed in previous
     * step and we want to use an alternative measure.
     */
    //TODO
